...
{
    public class ProjectGanttPresenter : lm.Comol.Core.DomainModel.Common.DomainPresenter
    {
           #region "Initialize"
            private ServiceProjectManagement service;
            public virtual BaseModuleManager CurrentManager { get; set; }
            private Int32 currentIdModule;
            protected virtual IViewProjectGantt View
            {
                get { return (IViewProjectGantt)base.View; }
            }
            private ServiceProjectManagement Service
            {
                get
            ...
            public ProjectGanttPresenter(iApplicationContext oContext)
                : base(oContext)
            {
                this.CurrentManager = new BaseModuleManager(oContext);
            }
            public ProjectGanttPresenter(iApplicationContext oContext, IViewProjectGantt view)
                : base(oContext, view)
            {
                this.CurrentManager = new BaseModuleManager(oContext);
            }
        ...
        public void InitView()
        {
            ...
            else
            {
                ...
                SetBackUrls(fromPage, View.PreloadIdContainerCommunity, project, cContext);
                if (project == null)
                {
                ...
                else
                {
                    ...
                    {
                        View.AllowSave = (fromPage != PageListType.ProjectDashboardResource && fromPage != PageListType.DashboardResource && fromPage != PageListType.ListResource);
                        View.LoadProjectDateInfo(project, (fromPage != PageListType.ProjectDashboardResource && fromPage != PageListType.DashboardResource && fromPage != PageListType.ListResource));
                        if (project.EndDate.HasValue)
                            View.LoadGantt(project);
                        else
                            View.DisplayProjectWithNoTasks();
                        ...
                        View.SetEditProjectUrl(RootObject.EditProject(project.Id, project.IdCommunity, project.isPortal, project.isPersonal, fromPage, View.IdContainerCommunity));
                        if (!View.AllowSave)
