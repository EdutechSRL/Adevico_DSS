...
#Region "Button Click Events"
    ...
    Protected Sub tornaAllaLista()
        Try
            Me.RedirectToUrl(RootObject.QuestionariGestioneList & "&" & qs_questType & Questionario.TipoQuestionario.Meeting)
        Catch ex As Exception
            BindDati()
        End Try
    End Sub
...
#End Region
    ...
    Public Overrides Sub BindDati()
        caricaDati()
        If DALRisposte.countRisposteBYIDQuestionario(Me.QuestionarioCorrente.id) > 0 Then
            RedirectToUrl(RootObject.QuestionarioAdmin & "?type=6")
        End If
        SetCampiVisibili()
        ...
        End If
        If oPagina Is Nothing OrElse oPagina.id = 0 Then
            setPagina(oPagina)
            'Else
            '    If oPagina.id = 0 Then
            '        setPagina(oPagina)
    ...
    Public Overrides Sub SetCultureSettings()
        MyBase.SetCulture("pg_MeetingAdmin", "Questionari")
    End Sub
