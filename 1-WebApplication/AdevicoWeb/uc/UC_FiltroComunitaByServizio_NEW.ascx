<%@ Control Language="vb" AutoEventWireup="false" Codebehind="UC_FiltroComunitaByServizio_NEW.ascx.vb" Inherits="Comunita_OnLine.UC_FiltroComunitaByServizio_NEW" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
function ToggleTreeView(){
	...
	function ProcessClientClick(node){ 
		if (node.Value =="" || node.Category < 1)
			return false
		document.getElementById("<%= HDN_valoreDestinatario.ClientID %>").value=node.Value
...
<asp:table ID="TBLDdlCom" Runat="server" >
	<asp:TableRow>
		...
		<div id="divDestinatario" style="position:absolute; visibility: hidden; border: solid 1px; width:<%=me.RDTcomunita.Width%>px; background: white; height: 250px; zIndex=1;">
			<radTree:RadTreeView ID="RDTcomunita" runat="server" Height="250px" Width="550px" AutoPostBack="True"
