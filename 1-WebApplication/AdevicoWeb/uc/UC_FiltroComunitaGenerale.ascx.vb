...
Public Class UC_FiltroComunitaGenerale
...
#Region "Bind_Filtri"
    Public Sub SetupControl(Optional ByVal ComunitaAttualeID As Integer = -1, Optional ByVal ComunitaAttualePercorso As String = "")
        ...
        Try
            ...
            oSelectedNode = Me.RDTcomunita.FindNodeByValue(Me.HDN_ComunitaAttualePercorso.Value)
...
TBRorgnCorsi.Visible = False
    ...
    Private Sub Bind_TipiComunita()
        ...
        Try
            oDataSet = COL_Tipo_Comunita.ElencaForFiltri(Session("LinguaID"), True, 0)
    ...
    Private Function FiltraggioDati() As DataSet
        ...
        Try
            ...
            End Try
            Try
                AAid = Me.DDLannoAccademico.SelectedValue
            Catch ex As Exception
                AAid = -1
            End Try
            Try
            ...
            Try
                If ComunitaPath = "" Then
                    ComunitaPadreID = -1
                Else
                    Dim ElencoPadriID As String()
                    ElencoPadriID = ComunitaPath.Split(".")
                    If ElencoPadriID.Length <= 3 Then
                        ComunitaPadreID = 0
                    Else
                        ComunitaPadreID = ElencoPadriID(ElencoPadriID.Length - 3)
                    End If
                End If
            ...
            oDataset = oTreeComunita.AlberoGeneraleForFiltri(Session("LinguaID"), Me.HDN_ComunitaAttualeID.Value, Me.oResourceUCcomunita, ImageBaseDir, TipocomunitaID, FacoltaID, , , , ComunitaPadreID, ComunitaPath, oFiltroTipoRicerca, valore, StatusID, , , True)
...
#Region "AlberoComunità"
        ...
        Try
            ...
            Else
                ...
                For Each dbRow In oDataset.Tables(0).Rows
                    If dbRow("ALCM_PadreVirtuale_ID") = 0 Or IDcorrente = dbRow("CMNT_ID") Then
    ...
    Private Function CreateNode(ByVal dbRow As DataRow, ByVal expanded As Boolean, ByVal nodeFather As RadTreeNode) As RadTreeNode
        ...
        Dim CMNT_Nome, CMNT_NomeVisibile As String, CMNT_REALpath As String = "", CMNT_path As String = ""
