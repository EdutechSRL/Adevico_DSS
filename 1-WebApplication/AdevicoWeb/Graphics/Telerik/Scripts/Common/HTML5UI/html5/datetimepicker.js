...
X._oldText=V.val();
z.notify(X);
},options:{name:"DateTimePicker",value:null,format:"",timeFormat:"",culture:"",parseFormats:[],dates:[],min:new m(C),max:new m(B),interval:30,height:200,footer:"",start:D,depth:D,animation:{},month:{},ARIATemplate:'Current focused date is #=kendo.toString(data.current, "d")#'},events:[H,l,j],setOptions:function(X){var Y=this,aa=Y._value,U=Y.dateView.options,Z=Y.timeView.options,W,V,T;
S.fn.setOptions.call(Y,X);
X=Y.options;
X.min=W=I(X.min);
X.max=V=I(X.max);
F(X);
T=X.value||Y._value||Y.dateView._current;
if(W&&!x(W,T)){W=new m(C);
}if(V&&!x(V,T)){V=new m(B);
}Y.dateView.setOptions(X);
Y.timeView.setOptions(r({},X,{format:X.timeFormat,min:W,max:V}));
if(aa){Y.element.val(z.toString(aa,X.format,X.culture));
Y._updateARIA(aa);
...
}function F(T){var U=z.getCulture(T.culture).calendars.standard.patterns,V;
T.format=s(T.format||U.g);
T.timeFormat=V=s(T.timeFormat||U.t);
z.DateView.normalize(T);
if(a.inArray(V,T.parseFormats)===-1){T.parseFormats.splice(1,0,V);
}}Q.plugin(n);
})(window.kendo.jQuery);
return window.kendo;
