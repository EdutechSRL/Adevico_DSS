...
}},select:function(Y){var aa=this,Z=aa.options,X=aa._current;
if(Y instanceof Date){Y=z.toString(Y,Z.format,Z.culture);
}if(typeof Y==="string"){if(!X||X.text()!==Y){Y=a.grep(aa.ul[0].childNodes,function(ab){return(ab.textContent||ab.innerText)==Y;
});
Y=Y[0]?Y:null;
}else{Y=X;
}}aa.current(Y);
...
}},_click:function(X){var Z=this,Y=a(X.currentTarget);
if(!X.isDefaultPrevented()){Z.select(Y);
Z.options.change(Y.text(),true);
Z.close();
