...
},_validate:function(x){var B=this,z,C=x.value,A=B._validationEventInProgress,E={},v=o.attr("bind"),y=x.field.replace(p,"\\$1"),w="checked:"+y,D="value:"+y;
E[x.field]=x.value;
z=a(":input["+v+'*="'+D+'"],:input['+v+'*="'+w+'"]',B.element).filter("["+o.attr("validate")+"!='false']");
if(z.length>1){z=z.filter(function(){var G=a(this);
var F=G.attr(v).split(",");
var H=l(D,F)>=0||l(w,F)>=0;
return H&&(!G.is(":radio")||G.val()==C);
});
}try{B._validationEventInProgress=true;
if(!B.validatable.validateInput(z)||(!A&&B.trigger(c,{values:E}))){x.preventDefault();
...
b(E,G);
H.editor(w,E);
}if(H.options.target){H.angular("compile",function(){return{elements:v,data:[{dataItem:D}]};
});
}if(!C){F=D.fields||D;
for(x in F){b(F[x],G);
}}e(v);
