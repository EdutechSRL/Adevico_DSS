...
this._originalHtml=this._originalHtml.replace(this._ignoreOtherTags,this._originalHtml);
...
d.insertRow(-1);
var b=d.rows[0].insertCell(-1);
b.style.width="100%";
var a=c.createElement("button");
a.className="reAjaxspell_button";
b=d.rows[0].insertCell(-1);
this._finishButton=a.cloneNode(true);
...
b=d.rows[0].insertCell(-1);
this._cancelButton=a.cloneNode(true);
...
},_enableButtons:function(c){var a=this._topElement.getElementsByTagName("button");
for(var b=0;
b<a.length;
b++){a[b].disabled=!c;
...
Telerik.Web.UI.Editor.SpellEngineUI=function(a,b){this._editor=a;
...
if(c._suggestionDropdown){var j=c._suggestionDropdown;
var h=(j.isExpanded());
var i=g.keyCode;
if(h){if(38==i){j.selectPreviousItem();
f=true;
}else{if(40==i){j.selectNextItem();
f=true;
}else{if(13==i){j.hide();
if(j.get_activeIndex){j.set_selectedIndex(j.get_activeIndex());
}c._onDropDownValueSelected(j);
try{g.keyCode=123;
}catch(g){}f=true;
}else{if(27==i){j.hide();
}}}}f=true;
...
g.attachEventHandler("click",c.OnMouseHandler);
g.attachEventHandler("keydown",c.OnKeyDownHandler);
g.attachEventHandler("contextmenu",Function.createDelegate(c,c.OnContextMenu));
...
var m=this;
var k=null;
function d(){var p=h.get_contentArea().getElementsByTagName("SPAN");
var e=0;
var o=p[e];
k=h.getSelection().getRange();
while(o!=null){if(m.isHighlightedWord(o)){var q=null;
if(h.isIE){if(k.duplicate){tempRange=k.duplicate();
}else{tempRange=h.get_contentArea().createTextRange();
}if(tempRange.moveToElementText){tempRange.moveToElementText(o);
}if(!k.compareEndPoints){break;
}q=k.compareEndPoints("EndToStart",tempRange);
if(0==q){j=o;
break;
}}else{if(k){tempRange=k.cloneRange();
tempRange.selectNodeContents(o);
q=k.compareBoundaryPoints(Range.END_TO_START,tempRange);
}}if(a==q){j=o;
break;
}}e++;
o=p[e];
...
}var i=g?g:h.innerHTML.replace(/<\/?[^>]*>/ig,"");
var a=g?true:false;
...
range.findText(j.data);
range.select();
}catch(c){return;
}}}else{this._editor.selectElement(j,false);
...
},GetBeforeText:function(d){var f=0;
var e=this.GetWordStartIndex(this.badWords[d]);
if(d!=0){badWordBefore=this.badWords[d-1];
f=this.GetWordEndCharIndex(badWordBefore);
}return this.Content.substring(f,e);
...
b.onclick=new Function("this.focus();");
