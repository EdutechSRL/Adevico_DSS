...
var c=prompt(d,"CustomWindow");
if(c){var b=document.createElement("option");
b.innerHTML=c;
...
},_insertClickHandler:function(b){if(!this._originalImage&&this._imageSrc.value){this._originalImage=document.createElement("img");
...
this._imageWidth.value=(g.indexOf("px")!=-1)?parseInt(g):g;
var d=this._getOriginalDimension(e,"height",e.height,e.offsetHeight);
this._imageHeight.value=(d.indexOf("px")!=-1)?parseInt(d):d;
...
}},_setDimensionAttribute:function(c,a,d){if(!parseInt(d)){d="";
}c.removeAttribute(a);
...
if(!g){g=this._tableToModify=document.createElement("table");
this._originalRowsCount=0;
this._originalColsCount=0;
var f=parseInt(this._rowsCount.value);
...
},_getRowColSpan:function(b,a){if(b){var c=parseInt(b.getAttribute(a));
if(isNaN(c)){c=1;
}return c;
}return 0;
...
var a=parseInt(l.border);
this._borderWidth.value=isNaN(a)?"":a;
}},set_clientParameters:function(a){this._clientParameters=a;
},getModifiedTable:function(h){var g=parseInt(this._rowsCount.value);
var a=parseInt(this._colsCount.value);
if(!h&&!(isNaN(g)||isNaN(a))){h=document.createElement("table");
this._originalRowsCount=0;
this._originalColsCount=0;
}if(!h){return;
...
var f=parseInt(this._cellSpacing.value);
if(!isNaN(parseInt(f))){this._setAttribValue(h,"cellSpacing",f>=0?f:"",(f>=0));
}var c=parseInt(this._cellPadding.value);
if(!isNaN(parseInt(c))){this._setAttribValue(h,"cellPadding",c>=0?c:"",(c>=0));
}var b=parseInt(this._borderWidth.value);
if(!isNaN(parseInt(b))){this._setAttribValue(h,"border",b>=0?b:"",(b>=0));
}return h;
