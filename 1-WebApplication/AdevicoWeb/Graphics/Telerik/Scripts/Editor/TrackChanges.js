...
Type.registerNamespace("Telerik.Web.UI.Editor");
(function(d,a,c,b){a.extend(c.DefaultToolAdapter.prototype,{onContextMenu:function(i){if(!this._contextMenusEnabled){return;
}var j=this.get_editor();
if(j.get_mode()!=d.EditModes.Design){return;
}this.createContextMenus();
var k=j.getSelectedElement()?j.getSelectedElement():i.srcElement;
var l=k.tagName;
var f=this._contextMenus[l];
if(l=="TD"){f=null;
}if("TH"==l&&!f){f=this._contextMenus.TD;
}var g="";
if(!f){var m=c.Utils.getElementParentByTag(k,"A");
if(m){var h=j.getSelectedElement();
var o=new RegExp("(</?(sub|sup|em|strike|strong|u|del)[^>]*("+b.Attribute.Author+"|"+b.Attribute.Cite+")=?)","i");
if($telerik.getOuterHtml(h).match(o)){m=null;
}}if(!m){m=c.Utils.getElementParentByTag(k,"TD");
}if(!m){m=c.Utils.getElementParentByTag(k,"TABLE");
}if(m){g=m.tagName;
k=m;
}else{g="*";
}f=this._contextMenus[g];
}if(f&&l=="IMG"&&k&&"true"==k.getAttribute("isflash")){f=null;
}var n=c.TrackChangesUtils.findTrackChangesParentElement(k);
if(n){if(n.Type==c.TrackChangesElementType.Comment){f=this.createContextMenuWithTrackChangeCommentItems(g);
}else{f=this.createContextMenuWithTrackChangeItems(g);
}}else{if(l!="TABLE"&&l!="TH"&&l!="TR"&&l!="IMG"&&l!="A"){f=this.createContextMenuWithTrackChangeDefaultItems(g);
}}if(f){f.set_eventObject(i);
f.show();
...
Type.registerNamespace("Telerik.Web.UI.Editor");
(function(a,b,d,c){b.IndentationTracker=function(e){b.IndentationTracker.initializeBase(this,[e]);
};
b.IndentationTracker.prototype={trackFormatting:function(g,e){var m=this;
var i=g.node;
if(!m.isListItem&&!m.isTextNode&&!g.originalValue&&!i.style.marginLeft){if(g.isWrapper){d.removeNode(i);
}return;
}if(!m.isTracked){m._savePreviousState(g);
c.setTrackChangesAttributes(i,e,m.author,m.userCssId,m.timestamp);
return;
}var k=c.getTrackChangesCommand(i);
var f=i.getAttribute("cssproperty");
var h=i.getAttribute("iswrapper");
var j=i.getAttribute("originalvalue")||"";
if(((k=="Indent"&&e=="Outdent")||(k=="Outdent"&&e=="Indent"))){if(h&&!f&&i.style.marginLeft==j){var l=new b.DomRange.rangeFromWindow(i.ownerDocument.defaultView||window);
d.removeNode(i);
l.select();
}else{if(m.isListItem||(f&&i.style[f]==j)){c.removeNodeTracking(i);
}}}},acceptNodeChanges:function(g,e){var h=this;
if(/^insert(un)orderedList/i.test(g.getAttribute("innercommand")||"")){var f=new b.InsertListTracker(h._editor);
return f.acceptNodeChanges(g,g.getAttribute("innercommand"));
}if(d.isTag(g,"li")){h._acceptListItemChanges(g,e);
}else{b.IndentationTracker.callBaseMethod(h,"acceptNodeChanges",[g]);
}},_acceptListItemChanges:function(g,f){var h=g.getAttribute("previousoutdent"),i="trackingstate",j=g.getAttribute("timestamp").toString();
if(h){h=h.toString();
j=j+"|"+h.replace(/[,]/gm,"|");
g.removeAttribute("previousoutdent");
}a(this._editor.get_contentArea()).find("*").filter(function(){var k=this.getAttribute?this.getAttribute(i):null;
return k&&new RegExp(j,"").test(k);
}).removeAttr(i);
if(f=="Indent"&&c.isTrackChangeElement(g.parentNode)&&c.getTrackChangesCommand(g.parentNode)==f){c.removeNodeTracking(g.parentNode);
}var e=c.getSameTrackingSiblings(g);
e.each(function(k,l){c.removeNodeTracking(l);
});
},_executeOppositeCommand:function(h,g){var i=g=="Indent"?"Outdent":"Indent",f=new b[i+"Command"](this._editor,{title:i});
var e=c.getSameTrackingSiblings(h);
e.each(function(j,k){c.removeNodeTracking(k);
f.executeOnNode(k);
});
},restoreNode:function(e){this._restoreFormatting(e);
c.removeNodeTracking(e);
}};
...
Type.registerNamespace("Telerik.Web.UI.Editor");
(function(a,b,e,d){var c=/^insert(un)?orderedList/i;
b.OutdentTracker=function(f){b.OutdentTracker.initializeBase(this,[f]);
this.stateAttrName="trackingstate";
};
b.OutdentTracker.prototype={trackNodeFormatting:function(i,g){var m=this,k=i.node,f=m.author,q=m.userCssId,p=m.timestamp;
if(c.test(m._innerCommand||"")){var j=new b.InsertListTracker(m._editor);
j._innerCommand=m._innerCommand;
j.timestamp=m.timestamp;
return j.trackNodeFormatting(i,g);
}if(!i||(!i.parentNode&&!i.previousSibling&&!i.nextSibling)){b.OutdentTracker.callBaseMethod(m,"trackNodeFormatting",[i,g]);
return;
}if(e.isList(k)&&!a(k).children().length){return;
}if(m.isListItem){if(m.isTracked&&d.getTrackChangesCommand(k)=="Indent"){d.removeNodeTracking(k);
return;
}d.setTrackChangesAttributes(k,g,f,q,p);
return;
}d.setTrackChangesAttributes(k,g,f,q,p);
var n=i.previousSibling||i.firstItem,o=this.stateAttrName;
var h,l;
if(n){h=p+"s";
l=a(n).attr(o)||"";
if(l){a(n).attr(o,h+","+l);
...
}d.removeNodeTracking(k);
},rejectNodeChanges:function(h){var i=this;
if(c.test(h.getAttribute("innercommand")||"")){var f=new b.InsertListTracker(i._editor);
return f.rejectNodeChanges(h,h.getAttribute("innercommand"));
}if(h.getAttribute("iswrapper")){b.OutdentTracker.callBaseMethod(i,"rejectNodeChanges",[h]);
return;
}var g=e.isTag(h,"li");
if(!g){return i.restoreNode(h);
}i._executeOppositeCommand(h,i.commandName);
},_executeOppositeCommand:function(h,f){var i=parseInt(h.getAttribute("timestamp"),10);
b.OutdentTracker.callBaseMethod(this,"_executeOppositeCommand",[h,f]);
var g=e.isTag(h,"li");
if(!g){return this.restoreNode(h);
}this._rejectOutdentedList(h,f,i);
},_rejectOutdentedList:function(j,h,p){var n=p.toString();
var m=this.stateAttrName;
var i=this._editor.get_contentArea();
var f=a(i).find("*").filter(function(){var q=this.getAttribute?this.getAttribute(m):null;
return q&&new RegExp(n,"").test(q);
});
var l=this._getElementByAttribute(f,m,"s");
var o=this._getElementByAttribute(f,m,"t");
f.removeAttr(m);
var g;
if(e.isList(o)&&e.isList(l)&&o!=l){g=a(o);
g.children().each(function(q,r){l.appendChild(r);
});
g.remove();
}else{if(e.isTag(l,"li")&&e.isList(o)){g=a(o);
g.children().insertAfter(l);
g.remove();
}}var k=j.getAttribute("previousoutdent");
if(!k){return;
}this._updateCommandAttributes(j,k);
this._executeOppositeCommand(j,"Outdent");
},_getElementByAttribute:function(f,g,h){return f.filter(function(){var i=this.getAttribute(g);
return RegExp(h,"").test(i);
})[0];
},_updateCommandAttributes:function(f,h){h=h.toString();
var j=h.split(",");
var i=this._getLastTimestamp(j);
if(!i||!/^[0-9]{13}$/.test(i)){return;
}f.setAttribute("timestamp",i);
d.setTrackChangesCommand(f,"Outdent");
Array.remove(j,i);
var g=j.join(",");
if(g){f.setAttribute("previousoutdent",g);
}else{f.removeAttribute("previousoutdent");
}},_getLastTimestamp:function(j){if(!j||!j.length){return null;
}var h=parseInt(j[0],10);
for(var g=0;
g<j.length;
g++){var f=parseInt(j[g],10);
h=Math.max(h,f);
}return h.toString();
}};
