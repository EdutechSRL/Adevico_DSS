<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/AjaxPortal.Master" Codebehind="Iscrizione_Gruppi.aspx.vb" Inherits="Comunita_OnLine.Iscrizione_Gruppi" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
	...
	function SelectAll( SelectAllBox ){
		...
		var TBcheckbox;
		//eval('HDNcorsiSelezionati= this.document.forms[0].HDNcomunitaSelezionate')
        //eval('HDNcorsiSelezionati=<%=Me.HDNcomunitaSelezionate.ClientID%>')
        HDNcorsiSelezionati = this.document.getElementById('<%=Me.HDNcomunitaSelezionate.ClientID%>');
		HDNcorsiSelezionati.value = ""
		for(i=0;i< document.forms[0].length; i++){ 
			e=document.forms[0].elements[i];
			if ( e.type=='checkbox' && e.name.indexOf("CBcorso") != -1 ){
				e.checked= actVar ;
				if (e.checked==true)
					if (HDNcorsiSelezionati.value == "")
						HDNcorsiSelezionati.value = ',' + e.value+','
					else
						HDNcorsiSelezionati.value = HDNcorsiSelezionati.value + e.value +','
			}
		}
	}
			
	function HasCorsiSelezionati(conferma,Messaggio,MessaggioConferma){
		var HIDcheckbox,selezionati;
		//eval('HIDcheckbox= this.document.forms[0].HDNcomunitaSelezionate')
        //eval('HIDcheckbox=<%=Me.HDNcomunitaSelezionate.ClientID%>')
        HIDcheckbox = this.document.getElementById('<%=Me.HDNcomunitaSelezionate.ClientID%>');
					...
					<tr>
						...
						<td align=right>
							<asp:Panel ID="PNLmenu" Runat=server HorizontalAlign=Right Visible=true>
								<asp:linkbutton  ID="LNBiscriviMultipli" Enabled=False CssClass="LINK_MENU" Runat="server"   Text="Iscrivi ai selzionati"></asp:linkbutton>
							</asp:Panel>
							...
							<asp:TableCell CssClass="Filtro_CellFiltri" ColumnSpan=2 Width=900px HorizontalAlign=center>
								<asp:Table Runat=server ID="TBLfiltro" CellPadding=1 CellSpacing=0 Width="900px" HorizontalAlign=center>
									...
									<asp:TableRow  Height=30px>
										<asp:TableCell HorizontalAlign=right Height=30px ColumnSpan=2>
						...
						<asp:TableRow>
							<asp:TableCell ColumnSpan=2>
								<asp:datagrid 
									...
									<Columns>
										<asp:TemplateColumn HeaderText="Tipo" ItemStyle-Width="40" SortExpression="TPCM_Descrizione" ItemStyle-CssClass=ROW_TD_Small_Center>
											<ItemTemplate>
...
<HTML>
  <head runat="server">
		<title>Comunità On Line - Iscrizione Gruppi di Lavoro</title>
