...
#Region "Bind_Dati"
	...
	Private Function FiltraggioDati(Optional ByVal ApplicaFiltri As Boolean = False) As DataSet
		...
		Dim i, totale, totaleHistory, TPCM_ID, PRSN_ID As Integer
		Dim oDataset As New DataSet
		Try
			...
			Select Case Me.HDN_filtroTipoComunitaID.Value
                Case Main.TipoComunitaStandard.Organizzazione
                    oDataset = oComunita.RicercaComunitaOrganizzazioni(oFiltroIscrizione, Session("LinguaID"), oPersona.ID, oFiltroTipoRicerca, oFiltroLettera, valore, StatusID)

				Case Else
					oDataset = oComunita.RicercaComunita(oFiltroIscrizione, Session("LinguaID"), FacoltaID, ComunitaPadreID, oPersona.ID, oFiltroTipoRicerca, oFiltroLettera, valore, TipocomunitaID, , , , , StatusID)
			End Select
	...
	Private Sub DGComunita_ItemCreated(ByVal sender As Object, ByVal e As DataGridItemEventArgs) Handles DGcomunita.ItemCreated
		...
		If (e.Item.ItemType = ListItemType.Item) OrElse (e.Item.ItemType = ListItemType.AlternatingItem) Then
			Try
                ...
                forSubscription = (Me.RBLricercaByIscrizione.SelectedIndex = 0)
				...
				Try
					...
					If visibileEntra Then
						If IsNothing(oLNBentra) = False Then
							...
							If e.Item.DataItem("CMNT_Bloccata") Then
								oResource.setLinkButton(oLNBentra, True, False, True)
							Else
								oResource.setLinkButton(oLNBentra, True, False)
							End If

							oLNBentra.Text = Replace(oLNBentra.Text, "#%%#", e.Item.DataItem("CMNT_EstesoNoSpan"))
							oStringa = oResource.getValue(oLNBentra.ID & ".status")
                    ...
                    Try
                        ...
                        If IsNothing(oLabelNews) = False AndAlso IsNothing(oLiteralNews) = False Then
                            ...
                            oLabelNews.Visible = (oCurrent.Count > 0) AndAlso Not CBool(e.Item.DataItem("CMNT_Bloccata")) AndAlso hasAccesso AndAlso visibileEntra
                            ...
                            If oCurrent.Count > 0 Then
                                ...
                                If Url <> "" Then
                                    Me.oResource.setLiteral(oLiteralNews)
...
#Region "Internazionalizzazione"
    ...
    Private Sub SetupInternazionalizzazione()
        With oResource
            ...
            '     .setButtonByValue(Me.BTNMessaggi, Me.stringaMessaggio.ok, True, True)
        ...
        If Session("azione") = "iscrivi" Then
            ...
            Try
                ...
                If iResponse <> Main.ErroriIscrizioneComunita.IscrizioneAvvenuta And iResponse <> Main.ErroriIscrizioneComunita.IscrizioneInAttesa Then
                    Me.Reset_ToMessaggi()
                    Me.LBMessaggi.Text = Me.oResource.getValue("ErroriIscrizioneComunita." & CType(iResponse, Main.ErroriIscrizioneComunita))
                Else : Dim oServiceUtility As New SubscriptionNotificationUtility(Me.PageUtility)
                    If iResponse = Main.ErroriIscrizioneComunita.IscrizioneAvvenuta Then
                        oServiceUtility.NotifyAddSelfSubscription(Me.HDNcmnt_ID.Value, Me.PageUtility.CurrentUser.ID, Me.PageUtility.CurrentUser.Anagrafica)
                    Else
                        oServiceUtility.NotifyAddWaitingSubscription(Me.HDNcmnt_ID.Value, Me.PageUtility.CurrentUser.ID, Me.PageUtility.CurrentUser.Anagrafica)
