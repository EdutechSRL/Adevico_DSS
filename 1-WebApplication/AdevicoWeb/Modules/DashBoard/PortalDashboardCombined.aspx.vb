...
Public Class PortalDashboardCombined
...
#Region "Implements"
    Protected Friend Overrides Sub DisplaySessionTimeout()
        ...
        If IsNothing(settings) Then
            ...
            settings.View = DashboardViewType.Combined
            settings.GroupBy = CTRLdashboardTopBar.SelectedGroupBy
            settings.OrderBy = CTRLminiTile.CurrentOrderItemsBy
            settings.Ascending = (settings.OrderBy = OrderItemsBy.Name)
            settings.CombinedNoticeboard = CurrentDisplayNoticeboard
            settings.IdSelectedTag = CTRLminiTile.IdCurrentTag
            settings.IdSelectedTile = CTRLminiTile.IdCurrentTile
        End If
    ...
    Protected Friend Overrides Function GetCurrentView() As lm.Comol.Core.Dashboard.Domain.DashboardViewType
        Return lm.Comol.Core.Dashboard.Domain.DashboardViewType.Combined
    End Function
    ...
    Protected Friend Overrides Sub IntializeCombinedView(pSettings As litePageSettings, userSettings As UserCurrentSettings, items As List(Of dtoItemFilter(Of OrderItemsBy)), idDashboard As Long, ByVal moreTiles As Boolean, ByVal moreCommunities As Boolean, Optional idPreloadTile As Long = -1)
        ...
        CTRLminiTile.InitalizeControlForTile(pSettings, userSettings, items, idDashboard, moreTiles, moreCommunities, idPreloadTile)
    End Sub
...
#Region "internal"
    ...
    End Sub
    Private Sub CTRLtiles_SessionTimeout(ByVal url As String) Handles CTRLminiTile.SessionTimeout
