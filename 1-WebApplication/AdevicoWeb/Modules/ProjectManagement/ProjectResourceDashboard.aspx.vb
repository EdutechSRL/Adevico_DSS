...
Public Class ProjectResourceDashboard
...
#Region "Inherits"
    ...
    Public Overrides Sub SetInternazionalizzazione()
        With MyBase.Resource
            ...
            Master.ServiceTitle = .getValue("ServiceTitle.ProjectDashboard." & TabListItem.Resource.ToString)
            Master.ServiceTitleToolTip = .getValue("ServiceTitle.ProjectDashboard." & TabListItem.Resource.ToString & ".ToolTip")
...
#Region "Implements"
    Private Sub InitializeProjectTopControl(context As dtoProjectContext, idContainerCommunity As Integer, loadFromCookies As Boolean, tab As TabListItem, pContainer As PageContainerType, idProject As Long, Optional dGroupBy As ItemsGroupBy = ItemsGroupBy.None, Optional projectsStatus As ItemListStatus = ItemListStatus.All, Optional activitiesStatus As ItemListStatus = ItemListStatus.Late, Optional timeline As SummaryTimeLine = SummaryTimeLine.Week) Implements IViewProjectDashboard.InitializeProjectTopControl
        CTRLtopControls.InitializeControl(context, idContainerCommunity, loadFromCookies, tab, pContainer, PreloadFromPage, PageListType.ProjectDashboardResource, dGroupBy, ProjectFilterBy.All, projectsStatus, activitiesStatus, timeline, SummaryDisplay.Project, idProject, PreloadUserActivityStatus, PreloadActivityTimeline)
    End Sub
...
#Region "Internal"
    ...
    Private Sub CTRLtopControls_Initialized(filter As dtoItemsFilter) Handles CTRLtopControls.Initialized
        ...
        Select Case filter.GroupBy
            Case ItemsGroupBy.Plain
                ...
                CTRLlistPlain.InitializeControl(DashboardContext, IdContainerCommunity, filter, PageContainerType.ProjectDashboard, FromPage, PageListType.ProjectDashboardResource)
            Case ItemsGroupBy.EndDate
                CTRLlistPlain.IsInitialized = False

                CTRLlistTree.Visible = True
                CTRLlistTree.InitializeControl(DashboardContext, IdContainerCommunity, filter, PageContainerType.ProjectDashboard, FromPage, PageListType.ProjectDashboardResource)
    ...
    Private Sub CompletionSaved(savedTasks As Integer, unsavedTasks As Integer, tasks As Integer, updateSummary As Boolean)
        ...
        Dim message As String = PageContainerType.ProjectDashboard.ToString & "." & PageListType.ProjectDashboardResource.ToString() & ".CompletionSaved."
    ...
    Private Sub CompletionSaved(ByVal saved As Boolean, ByVal idTask As Long, ByVal taskname As String, updateSummary As Boolean)
        Me.CTRLmessages.Visible = True
        If (saved) Then
            CTRLmessages.InitializeControl(String.Format(Resource.getValue(PageContainerType.ProjectDashboard.ToString & "." & PageListType.ProjectDashboardResource.ToString() & ".CompletionSaved." & saved.ToString), taskname), Helpers.MessageType.success)
        Else
            CTRLmessages.InitializeControl(String.Format(Resource.getValue(PageContainerType.ProjectDashboard.ToString & "." & PageListType.ProjectDashboardResource.ToString() & ".CompletionSaved." & saved.ToString), taskname), Helpers.MessageType.error)
        End If
        UpdateContents(updateSummary)
    End Sub
