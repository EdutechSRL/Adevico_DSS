...
Public Class NoticeboardDisplayMessage
    ...
    Private WriteOnly Property ContainerName As String Implements IViewDisplayMessage.ContainerName
        Set(value As String)
            If String.IsNullOrEmpty(value) Then
                ...
                LBcommunityName.Text = ""
            Else
                Dim translated As String = Resource.getValue("ContainerName")
                If String.IsNullOrEmpty(translated) Then
                    translated = "Noticeboard - {0}"
                End If

                Me.TLtitleCommunityName.Text = String.Format(translated, value)
                LBcommunityName.Text = value
            End If
...
#Region "Inherits"
    ...
    Public Overrides Sub SetInternazionalizzazione()
        With Me.Resource
            ...
            .setLiteral(LTnoticeboardRenderTitle)
        End With
    End Sub
...
#End Region
    ...
    Private Sub DisplayMessage(message As NoticeboardMessage) Implements IViewDisplayMessage.DisplayMessage
        ...
        If Not IsNothing(message.CreatedBy) Then
            LBcreatedBy.Text = message.CreatedBy.SurnameAndName
        Else
            LBcreatedBy.Visible = False
        End If
        If message.CreatedOn.HasValue Then
            LBcreatedOn.Text = message.CreatedOn.Value.ToString(Resource.CultureInfo.DateTimeFormat.ShortDatePattern) & " " & message.CreatedOn.Value.ToString(Resource.CultureInfo.DateTimeFormat.ShortTimePattern)
        End If
    End Sub
