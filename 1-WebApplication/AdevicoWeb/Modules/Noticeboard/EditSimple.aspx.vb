...
Public Class EditSimpleNoticeboardMessage
...
#Region "Inherits"
    Public Overrides Sub BindDati()
        CurrentPresenter.InitView(PreloadedIdMessage, False, PreloadedIdCommunity, PreloadFromPortal)
    End Sub
...
#End Region
    ...
    Private Sub EditMessage(message As NoticeboardMessage, advancedEditor As Boolean) Implements IViewEditMessage.EditMessage
        MLVcontent.SetActiveView(VIWcontainer)
        If advancedEditor <> message.CreateByAdvancedEditor Then
            CTRLmessages.Visible = True
            CTRLmessages.InitializeControl(Resource.getValue("EditMessage.Advanced"), lm.Comol.Core.DomainModel.Helpers.MessageType.alert)
        End If

        CTRLeditor.InitializeControl(message)
        'If IsNothing(message) Then
        '    Me.CTRLeditor.Html = ""
        'Else
        '    Me.CTRLeditor.Html = message.PlainText
        '    If Not message.CreateByAdvancedEditor AndAlso Not IsNothing(message.StyleSettings) Then
        '        Me.CTRLeditor.FontSize = message.StyleSettings.FontSize
        '        Me.CTRLeditor.FontFace = message.StyleSettings.FontFamily
        '        Me.CTRLeditor.FontColor = message.StyleSettings.FontColor
        '        Me.CTRLeditor.BackGround = message.StyleSettings.BackgroundColor
        '        Me.CTRLeditor.Alignment = message.StyleSettings.TextAlign
        '    End If
        'End If
    End Sub
...
#Region "internal"
    ...
    Private Sub SaveMessage(Optional gotoManagement As Boolean = False)
        ...
        CurrentPresenter.SaveMessage(gotoManagement, IdCurrentMessage, False, IdCurrentCommunity, IsPortalPage, NoticeboardFilePath & IdCurrentCommunity.ToString & "\", PageUtility.ApplicationUrlBase, OLDpageUtility.ApplicationWorkingId, PageUtility.ApplicationUrlBase(True), CTRLeditor.Html, CTRLeditor.Html)
    End Sub
#End Region
