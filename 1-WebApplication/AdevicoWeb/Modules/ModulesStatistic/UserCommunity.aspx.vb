...
Partial Public Class UserCommunityStatistics
    Inherits MSpageBase
    Implements IViewUserCommunityStatistics


#Region "Private Property"
    Private _PageUtility As OLDpageUtility
    Private _Presenter As UserCommunityStatisticsPresenter
...
#Region "Base Context"
    Public Overloads ReadOnly Property BaseUrl() As String
        ...
        End Get
    End Property
    Public ReadOnly Property CurrentPresenter() As UserCommunityStatisticsPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New UserCommunityStatisticsPresenter(PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "View property"
    Private ReadOnly Property CurrentPageSize() As Integer Implements IViewUserCommunityStatistics.CurrentPageSize
        Get
            Return 30
        End Get
    End Property
   
    Private Property Pager() As lm.Comol.Core.DomainModel.PagerBase Implements IViewUserCommunityStatistics.Pager
        Get
        ...
        End Set
    End Property
    Private ReadOnly Property CurrentView() As StatisticView Implements IViewUserCommunityStatistics.CurrentView
        Get
            Return StatisticView.UserCommunity
        End Get
    End Property

    Private ReadOnly Property CurrentOrder() As StatisticOrder Implements IViewUserCommunityStatistics.CurrentOrder
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadedFromView() As StatisticView Implements IViewUserCommunityStatistics.PreloadedFromView
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadedBackUrl() As String Implements IViewUserCommunityStatistics.PreloadedBackUrl
        Get
            Return Request.QueryString("BackUrl")
        End Get
    End Property
    Public Function GetEncodedBackUrl() As String Implements IViewUserCommunityStatistics.GetEncodedBackUrl
    ...
    Private Property StatisticContext() As UsageContext Implements IViewUserCommunityStatistics.StatisticContext
        Get
...
#Region "Generici pagina"
    ...
    End Sub
#End Region
    Public Sub DisplaySessionTimeout() Implements IViewUserCommunityStatistics.DisplaySessionTimeout
    ...
    Private Sub NoPermissionToAccess() Implements IViewUserCommunityStatistics.NoPermissionToAccess
        Me.BindNoPermessi()
    End Sub
    Public Sub DisplayUnknownUser() Implements IViewUserCommunityStatistics.DisplayUnknownUser
        Me.MLVuser.SetActiveView(VIWunknown)

    End Sub

    Public Sub SetBackUrl(url As String) Implements IViewUserCommunityStatistics.SetBackUrl
    ...
    End Sub
    Private Sub LoadItems(ByVal statistics As dtoStatistic, ByVal ucontext As UsageContext, ByVal vPage As StatisticView, ByVal detailType As DetailViewType) Implements IViewUserCommunityStatistics.LoadItems
        Me.GDVstatistic.DataSource = statistics.Items
        Me.GDVstatistic.DataBind()
        Me.PGgrid.BaseNavigateUrl = BaseUrl & RootObject.UserCommunity(ucontext.UserID, ucontext.CommunityID, ucontext.Order, ucontext.Ascending, GetEncodedBackUrl(), PreloadedFromView)
    End Sub
    ...
    Public Sub LoadSummary(ByVal oSummary As dtoSummary, ByVal oType As IViewBaseStatistics.SummaryType) Implements IViewUserCommunityStatistics.LoadSummary
                    ...
                    If TypeOf (oControl) Is System.Web.UI.WebControls.LinkButton Then
                        ...
                        If GetOrderByString(oLinkbutton.CommandArgument) = Me.CurrentOrder Then
                            If Me.Ascending Then
                                oHYPdesc.NavigateUrl = BaseUrl & RootObject.UserCommunity(0, tmpContext.UserID, tmpContext.CommunityID, GetOrderByString(oLinkbutton.CommandArgument), False, GetEncodedBackUrl(), PreloadedFromView)
                                oCell.Controls.Add(oHYPdesc)
                            Else
                                oHYPasc.NavigateUrl = BaseUrl & RootObject.UserCommunity(0, tmpContext.UserID, tmpContext.CommunityID, GetOrderByString(oLinkbutton.CommandArgument), True, GetEncodedBackUrl(), PreloadedFromView)
