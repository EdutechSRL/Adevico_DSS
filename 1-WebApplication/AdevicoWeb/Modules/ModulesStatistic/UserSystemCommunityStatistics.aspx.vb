...
Partial Public Class UserSystemCommunityStatistics
    Inherits MSpageBase
    Implements IViewUserSystemCommunityStatistics
...
#Region "Private Property"
    ...
    Private _Presenter As UserSystemCommunityStatisticsPresenter
...
#Region "Base Context"
    Public Overloads ReadOnly Property BaseUrl() As String
        ...
        End Get
    End Property
    Public ReadOnly Property CurrentPresenter() As UserSystemCommunityStatisticsPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New UserSystemCommunityStatisticsPresenter(PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "View property"
    Private ReadOnly Property CurrentPageSize() As Integer Implements IViewUserSystemCommunityStatistics.CurrentPageSize
        Get
            Return 30
        End Get
    End Property
   
    Private Property Pager() As lm.Comol.Core.DomainModel.PagerBase Implements IViewUserSystemCommunityStatistics.Pager
        Get
        ...
        End Set
    End Property
    Private ReadOnly Property CurrentView() As StatisticView Implements IViewUserSystemCommunityStatistics.CurrentView
        Get
            Return StatisticView.UserSystem
        End Get
    End Property

    Private ReadOnly Property CurrentOrder() As StatisticOrder Implements IViewUserSystemCommunityStatistics.CurrentOrder
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadedFromView() As StatisticView Implements IViewUserSystemCommunityStatistics.PreloadedFromView
        Get
            Return lm.Comol.Core.DomainModel.Helpers.EnumParser(Of StatisticView).GetByString(Me.Request.QueryString("From"), StatisticView.MySystem)
        End Get
    End Property
    Private ReadOnly Property PreloadedBackUrl() As String Implements IViewUserSystemCommunityStatistics.PreloadedBackUrl
        Get
            Return Request.QueryString("BackUrl")
        End Get
    End Property
    Public Function GetEncodedBackUrl() As String Implements IViewUserSystemCommunityStatistics.GetEncodedBackUrl
    ...
    Private Property StatisticContext() As UsageContext Implements IViewUserSystemCommunityStatistics.StatisticContext
        Get
...
#Region "Generici pagina"
    ...
    End Sub
#End Region
    Public Sub DisplaySessionTimeout() Implements IViewUserSystemCommunityStatistics.DisplaySessionTimeout
    ...
    Private Sub NoPermissionToAccess() Implements IViewUserSystemCommunityStatistics.NoPermissionToAccess
        Me.BindNoPermessi()
    End Sub
    Public Sub DisplayUnknownUser() Implements IViewUserSystemCommunityStatistics.DisplayUnknownUser
        Me.MLVuser.SetActiveView(VIWunknown)

    End Sub

    Public Sub SetBackUrl(url As String) Implements IViewUserSystemCommunityStatistics.SetBackUrl
    ...
    End Sub
    Private Sub LoadItems(ByVal statistics As dtoStatistic, ByVal ucontext As UsageContext, ByVal vPage As StatisticView, ByVal detailType As DetailViewType) Implements IViewUserSystemCommunityStatistics.LoadItems
        Me.GDVstatistic.DataSource = statistics.Items
        Me.GDVstatistic.DataBind()
        Me.PGgrid.BaseNavigateUrl = BaseUrl & RootObject.UserSystemCommunityStatistics(ucontext.UserID, ucontext.CommunityID, ucontext.Order, ucontext.Ascending, GetEncodedBackUrl(), PreloadedFromView)
    End Sub
    ...
    Public Sub LoadSummary(ByVal oSummary As dtoSummary, ByVal oType As IViewBaseStatistics.SummaryType) Implements IViewUserSystemCommunityStatistics.LoadSummary
                    ...
                    If TypeOf (oControl) Is System.Web.UI.WebControls.LinkButton Then
                        ...
                        If GetOrderByString(oLinkbutton.CommandArgument) = Me.CurrentOrder Then
                            If Me.Ascending Then
                                oHYPdesc.NavigateUrl = BaseUrl & RootObject.UserSystemCommunityStatistics(0, tmpContext.UserID, tmpContext.CommunityID, GetOrderByString(oLinkbutton.CommandArgument), False, GetEncodedText(tmpContext.SearchBy), PreloadedFromView)
                                oCell.Controls.Add(oHYPdesc)
                            Else
                                oHYPasc.NavigateUrl = BaseUrl & RootObject.UserSystemCommunityStatistics(0, tmpContext.UserID, tmpContext.CommunityID, GetOrderByString(oLinkbutton.CommandArgument), True, GetEncodedText(tmpContext.SearchBy), PreloadedFromView)
