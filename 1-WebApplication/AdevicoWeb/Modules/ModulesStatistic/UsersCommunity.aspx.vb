...
Partial Public Class UsersCommunityStatistics
    Inherits MSpageBase
    Implements IViewUsersCommunityStatistics

#Region "Private Property"
    Private _PageUtility As OLDpageUtility
    Private _Presenter As UsersCommunityStatisticsPresenter
...
#Region "Base Context"
    Public Overloads ReadOnly Property BaseUrl() As String
        ...
        End Get
    End Property
    Public ReadOnly Property CurrentPresenter() As UsersCommunityStatisticsPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New UsersCommunityStatisticsPresenter(PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "View property"
    Private ReadOnly Property CurrentPageSize() As Integer Implements IViewUsersCommunityStatistics.CurrentPageSize
        Get
            Return 25
        End Get
    End Property
    Private ReadOnly Property Ascending() As Boolean Implements IViewUsersCommunityStatistics.Ascending
        Get
    ...
    Private ReadOnly Property PreloadedFromView() As StatisticView Implements IViewUsersCommunityStatistics.PreloadedFromView
        Get
            Return lm.Comol.Core.DomainModel.Helpers.EnumParser(Of StatisticView).GetByString(Me.Request.QueryString("From"), StatisticView.UsersCommunity)
        End Get
    End Property
    Private Property Pager() As lm.Comol.Core.DomainModel.PagerBase Implements IViewUsersCommunityStatistics.Pager
        Get
        ...
        End Set
    End Property
    Private ReadOnly Property CurrentView() As StatisticView Implements IViewUsersCommunityStatistics.CurrentView
        Get
            Return StatisticView.UsersCommunity
        End Get
    End Property

    Private ReadOnly Property CurrentOrder() As StatisticOrder Implements IViewUsersCommunityStatistics.CurrentOrder
        Get
            Return lm.Comol.Core.DomainModel.Helpers.EnumParser(Of StatisticOrder).GetByString(Me.Request.QueryString("Order"), StatisticOrder.Owner)
        End Get
    End Property
    Private ReadOnly Property PreloadedSearchBy() As String Implements IViewUsersCommunityStatistics.PreloadedSearchBy
        Get
            Return Request.QueryString("SearchBy")
        End Get
    End Property
    Private Property StatisticContext() As UsageContext Implements IViewUsersCommunityStatistics.StatisticContext
        Get
        ...
        End Set
    End Property
    Private Property CurrentSearchBy As String Implements IViewUsersCommunityStatistics.CurrentSearchBy
        Get
...
#Region "Generici pagina"
    ...
    Public Overrides Sub SetInternazionalizzazione()
        With Me.Resource
            ...
            .setHeaderGridView(Me.GDVstatistic, GridColumn.Owner, "Owner", True)
            ...
            .setLabel(LBsearcyByUsername_t)
            .setButton(BTNsearchByUserName)
    ...
    End Sub
#End Region
    Public Sub SetBackUrl(url As String) Implements IViewUsersCommunityStatistics.SetBackUrl
    ...
    End Sub
    Public Sub DisplaySessionTimeout() Implements IViewUsersCommunityStatistics.DisplaySessionTimeout
    ...
    Private Sub NoPermissionToAccess() Implements IViewUsersCommunityStatistics.NoPermissionToAccess
        Me.BindNoPermessi()
    End Sub
            ...
            If Not IsNothing(oTab) Then
                ...
                Select Case v
                    Case StatisticView.MyCommunity
                        oTab.NavigateUrl = BaseUrl & RootObject.MyCommunityStatistics(0, c.CommunityID, StatisticOrder.ModuleName, True, IIf(dView = StatisticView.UsersCommunity, StatisticView.MyCommunity, dView))
                    Case StatisticView.UsersCommunity
                        oTab.NavigateUrl = BaseUrl & RootObject.UsersCommunity(0, c.CommunityID, "", StatisticOrder.Owner, True, dView)
                    Case StatisticView.Community
                        oTab.NavigateUrl = BaseUrl & RootObject.CommunityStatistics(0, c.CommunityID, "", StatisticOrder.ModuleName, True, IIf(dView = StatisticView.UsersCommunity, StatisticView.Community, dView))
                End Select
            End If
        Next
        Me.TBSusageTime.Visible = (views.Count > 1)
    End Sub
    Private Sub LoadItems(ByVal statistics As dtoStatistic, ByVal ucontext As UsageContext, ByVal vPage As StatisticView, ByVal detailType As DetailViewType) Implements IViewUsersCommunityStatistics.LoadItems
        Me.GDVstatistic.DataSource = statistics.Items
        Me.GDVstatistic.DataBind()
        Me.PGgrid.BaseNavigateUrl = BaseUrl & RootObject.UsersCommunity(ucontext.CommunityID, GetEncodedText(ucontext.SearchBy), ucontext.Order, ucontext.Ascending, PreloadedFromView)
    End Sub
    ...
    Public Sub LoadSummary(ByVal oSummary As dtoSummary, ByVal oType As IViewBaseStatistics.SummaryType) Implements IViewUsersCommunityStatistics.LoadSummary
...
#Region "Control"
                    ...
                    If TypeOf (oControl) Is System.Web.UI.WebControls.LinkButton Then
                        ...
                        If GetOrderByString(oLinkbutton.CommandArgument) = Me.CurrentOrder Then
                            If Me.Ascending Then
                                oHYPdesc.NavigateUrl = BaseUrl & RootObject.UsersCommunity(0, tmpContext.CommunityID, GetEncodedText(tmpContext.SearchBy), GetOrderByString(oLinkbutton.CommandArgument), False, PreloadedFromView)
                                oCell.Controls.Add(oHYPdesc)
                            Else
                                oHYPasc.NavigateUrl = BaseUrl & RootObject.UsersCommunity(0, tmpContext.CommunityID, GetEncodedText(tmpContext.SearchBy), GetOrderByString(oLinkbutton.CommandArgument), True, PreloadedFromView)
    ...
    Private Sub GDVstatistic_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles GDVstatistic.Sorting
        ...
        e.Cancel = True
    End Sub
    Private Sub BTNsearch_Click(sender As Object, e As System.EventArgs) Handles BTNsearchByUserName.Click
        PageUtility.RedirectToUrl(RootObject.UsersCommunity(0, StatisticContext.CommunityID, Server.UrlEncode(CurrentSearchBy), StatisticOrder.Owner, True, PreloadedFromView))
    End Sub
#End Region
