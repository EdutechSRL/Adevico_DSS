...
Partial Public Class UserSystemStatistics
    Inherits MSpageBase
    Implements IViewUserSystemStatistics
...
#Region "Private Property"
    ...
    Private _Presenter As UserSystemStatisticsPresenter
...
#Region "Base Context"
    Public Overloads ReadOnly Property BaseUrl() As String
        ...
        End Get
    End Property
    Public ReadOnly Property CurrentPresenter() As UserSystemStatisticsPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New UserSystemStatisticsPresenter(PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "View property"
    Private ReadOnly Property CurrentPageSize() As Integer Implements IViewUserSystemStatistics.CurrentPageSize
        Get
            Return 25
        End Get
    End Property
    Private Property Pager() As lm.Comol.Core.DomainModel.PagerBase Implements IViewUserSystemStatistics.Pager
        Get
        ...
        End Set
    End Property
    Private ReadOnly Property CurrentView() As StatisticView Implements IViewUserSystemStatistics.CurrentView
        Get
            Return StatisticView.UserSystem
        End Get
    End Property

    Private ReadOnly Property CurrentOrder() As StatisticOrder Implements IViewUserSystemStatistics.CurrentOrder
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadedFrom() As StatisticView Implements IViewUserSystemStatistics.PreloadedFrom
        Get
            Return lm.Comol.Core.DomainModel.Helpers.EnumParser(Of StatisticView).GetByString(Me.Request.QueryString("From"), StatisticView.UsersSystem)
        End Get
    End Property
    Private ReadOnly Property PreloadedBackUrl() As String Implements IViewUserSystemStatistics.PreloadedBackUrl
        Get
            Return Request.QueryString("BackUrl")
        End Get
    End Property
    Public Function GetEncodedBackUrl() As String Implements IViewUserSystemStatistics.GetEncodedBackUrl
        If String.IsNullOrEmpty(PreloadedBackUrl) Then
            Return ""
        Else
            Return Server.UrlEncode(PreloadedBackUrl)
        End If
    End Function
   
    Private ReadOnly Property PreloadedSearchBy() As String Implements IViewUserSystemStatistics.PreloadedSearchBy
        Get
            Return Request.QueryString("SearchBy")
        End Get
    End Property
    Private Property StatisticContext() As UsageContext Implements IViewUserSystemStatistics.StatisticContext
        Get
        ...
        End Set
    End Property
    Private Property CurrentSearchBy As String Implements IViewUserSystemStatistics.CurrentSearchBy
        Get
...
#Region "Generici pagina"
    Public Overrides Sub SetInternazionalizzazione()
        With Me.Resource
            ...
            .setHyperLink(HYPbackHistory, PreloadedFrom.ToString, False, True)
        End With
    End Sub
    ...
    End Sub
#End Region
    Public Sub DisplaySessionTimeout() Implements IViewUserSystemStatistics.DisplaySessionTimeout
    ...
    Private Sub NoPermissionToAccess() Implements IViewUserSystemStatistics.NoPermissionToAccess
        Me.BindNoPermessi()
    End Sub
    Public Sub DisplayUnknownUser() Implements IViewUserSystemStatistics.DisplayUnknownUser
        Me.MLVuser.SetActiveView(VIWunknown)

    End Sub

    Public Sub SetBackUrl(url As String) Implements IViewUserSystemStatistics.SetBackUrl
    ...
    End Sub
    Private Sub LoadItems(ByVal statistics As dtoStatistic, ByVal ucontext As UsageContext, ByVal vPage As StatisticView, ByVal detailType As DetailViewType) Implements IViewUserSystemStatistics.LoadItems
        Me.GDVstatistic.DataSource = statistics.Items
        Me.GDVstatistic.DataBind()
        Me.PGgrid.BaseNavigateUrl = BaseUrl & RootObject.UserSystemStatistics(ucontext.UserID, GetEncodedText(ucontext.SearchBy), ucontext.Order, ucontext.Ascending, GetEncodedBackUrl(), PreloadedFrom)
    End Sub
    ...
    Public Sub LoadSummary(ByVal oSummary As dtoSummary, ByVal oType As IViewBaseStatistics.SummaryType) Implements IViewUserSystemStatistics.LoadSummary
...
#Region "Control"
                    ...
                    If TypeOf (oControl) Is System.Web.UI.WebControls.LinkButton Then
                        ...
                        If GetOrderByString(oLinkbutton.CommandArgument) = Me.CurrentOrder Then
                            If Me.Ascending Then
                                oHYPdesc.NavigateUrl = BaseUrl & RootObject.UserSystemStatistics(0, tmpContext.UserID, GetEncodedText(tmpContext.SearchBy), GetOrderByString(oLinkbutton.CommandArgument), False, GetEncodedBackUrl(), PreloadedFrom)
                                oCell.Controls.Add(oHYPdesc)
                            Else
                                oHYPasc.NavigateUrl = BaseUrl & RootObject.UserSystemStatistics(0, tmpContext.UserID, GetEncodedText(tmpContext.SearchBy), GetOrderByString(oLinkbutton.CommandArgument), True, GetEncodedBackUrl(), PreloadedFrom)
    ...
    Private Sub GDVstatistic_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GDVstatistic.RowDataBound
        If e.Row.RowType = ListItemType.Item Or e.Row.RowType = ListItemType.AlternatingItem Then
            ...
            If Not oItem.Subscribed Then
                Dim oLiteral As Literal = e.Row.FindControl("LTinfo")
                oLiteral.Text = Resource.getValue("Unsubscribed")
    ...
    Private Sub BTNsearchByCommunity_Click(sender As Object, e As System.EventArgs) Handles BTNsearchByCommunity.Click
        PageUtility.RedirectToUrl(RootObject.UserSystemStatistics(StatisticContext.UserID, GetEncodedText(CurrentSearchBy), StatisticOrder.UsageTime, False, Server.UrlEncode(PreloadedBackUrl), PreloadedFrom))
    End Sub
#End Region
