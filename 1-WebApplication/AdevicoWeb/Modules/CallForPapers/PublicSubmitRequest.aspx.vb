...
#Region "Implements"
    ...
    Private Property isAnonymousSubmission As Boolean Implements IViewBaseSubmission.isAnonymousSubmission
        Get
            Return ViewStateOrDefault("isAnonymousSubmission", True)
        End Get
...
#Region "Inherits"
    ...
    Public Overrides Sub SetInternazionalizzazione()
        With MyBase.Resource
            ...
            HYPlistFirstPage.Text = HYPlist.Text
            HYPlistFirstPage.ToolTip = HYPlist.ToolTip
...
#Region "Implements"
    ...
    Private Sub LoadError(errorView As SubmissionErrorView, idCommunity As Integer, idCall As Long, view As CallStatusForSubmitters) Implements IViewBaseSubmitCall.LoadError
        If MLVpreview.GetActiveView Is VIWcall Then
            ...
            Me.CTRLviewMessages.InitializeControl(Me.Resource.getValue("SubmissionErrorView." & CallForPaperType.RequestForMembership.ToString & "." & errorView.ToString()), Helpers.MessageType.error)

        Else
            Me.CTRLstartMessages.Visible = (errorView <> SubmissionErrorView.None)
            Me.CTRLstartMessages.InitializeControl(Me.Resource.getValue("SubmissionErrorView." & CallForPaperType.RequestForMembership.ToString & "." & errorView.ToString()), Helpers.MessageType.error)
        End If
    End Sub
    ...
    '    Me.CTRLstartMessages.InitializeControl(Me.Resource.getValue("SubmissionErrorView." & CallForPaperType.RequestForMembership.ToString & "." & SubmissionErrorView.SubmissionTimeExpired.ToString), Helpers.MessageType.info)
    'End Sub
    ...
    Private Sub SetContainerName(name As String, itemName As String) Implements IViewBaseSubmission.SetContainerName
        ...
        Dim title As String = ""
        If (CallType = CallForPaperType.None) Then
        Else
            translation = Me.Resource.getValue("serviceTitle.Submit." & CallType.ToString())
    ...
    Private Sub SetActionUrl(action As CallStandardAction, url As String) Implements IViewBaseSubmission.SetActionUrl
        Select Case action
            Case CallStandardAction.List
                ...
                HYPlistFirstPage.NavigateUrl = BaseUrl & url
                Me.HYPlistFirstPage.Visible = True
        End Select
    End Sub
    ...
    Protected Sub RPTfields_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.RepeaterItemEventArgs)
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            If inputField.CurrentError <> FieldError.None OrElse field.FieldError <> FieldError.None Then
                inputField.InitializeControl(IdCall, IdSubmission, CallRepository, field, Not AllowSave, True, field.FieldError)
            Else
                inputField.InitializeControl(IdCall, IdSubmission, CallRepository, field, Not AllowSave, True)
            End If
        End If
    End Sub
...
#End Region
    ...
    Private Sub PublicSubmitRequest_PreRender(sender As Object, e As System.EventArgs) Handles Me.PreRender
