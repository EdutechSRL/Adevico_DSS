...
#End Region
    ...
    End Sub
    Private Sub LoadSubmissionInfo(callName As String, submitterName As String, submittedOn As Date?, submittedBy As String, committees As List(Of lm.Comol.Modules.CallForPapers.Domain.Evaluation.dtoCommitteeEvaluationInfo), idCommittee As Long, isPresident As Boolean, unused As Integer) Implements IViewEvaluationSummary.LoadSubmissionInfo
            ...
            Else
                ...
                If dto.Status = EvaluationStatus.EvaluatorReplacement OrElse dto.Status = EvaluationStatus.Invalidated Then
                    If dto.Values.Where(Function(v) Not v.IsValueEmpty).Any() Then
                        ...
                        oHyperLink.NavigateUrl = BaseUrl & RootObject.ViewUserEvaluations(dto.IdEvaluator, dto.IdSubmission, IdCall, IdCallCommunity, AdvCommissionId)
                    Else
                        oLabel.Text = "//"
                    End If
                    oGeneric.Attributes.Add("class", oGeneric.Attributes("class") & " gray")
