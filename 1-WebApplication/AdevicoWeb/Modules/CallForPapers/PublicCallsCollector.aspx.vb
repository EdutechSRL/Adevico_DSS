...
Public Class PublicCallsCollector
    Inherits PageBase
    Implements IViewPublicCallsList



#Region "Context"
    Private _Presenter As PublicCallsListPresenter
    Private ReadOnly Property CurrentPresenter() As PublicCallsListPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New PublicCallsListPresenter(Me.PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "Implements"
    Private ReadOnly Property Portalname As String Implements IViewPublicCallsList.Portalname
        Get
            Return Resource.getValue("Portalname")
        End Get
    End Property
    Private ReadOnly Property PreloadIdCommunity As Integer Implements IViewPublicCallsList.PreloadIdCommunity
        Get
        ...
        End Get
    End Property
    Private Property IdCallCommunity As Integer Implements IViewPublicCallsList.IdCallCommunity
        Get
        ...
        End Set
    End Property
    Private Property Pager As lm.Comol.Core.DomainModel.PagerBase Implements IViewPublicCallsList.Pager
        Get
...
#Region "Implements"
    Private Sub InitializeSkin(skin As lm.Comol.Core.DomainModel.Helpers.ExternalPageContext) Implements IViewBasePublicCallList(Of lm.Comol.Modules.CallForPapers.Domain.dtoCallItemPermission).InitializeSkin
        Me.Master.InitializeMaster(skin)
    End Sub
    Private Sub SetContainerName(idCommunity As Integer, name As String) Implements IViewPublicCallsList.SetContainerName
        Dim title As String = Me.Resource.getValue("serviceTitle." & CallStandardAction.List.ToString() & "." & CallForPaperType.CallForBids.ToString())
        Master.ServiceTitle = title
        If idCommunity = 0 Then
            Master.ServiceTitleToolTip = title
        Else
            Dim tooltip As String = Me.Resource.getValue("serviceTitle.Community." & CallStandardAction.List.ToString() & "." & CallForPaperType.CallForBids.ToString())
            If Not String.IsNullOrEmpty(tooltip) Then
                Master.ServiceTitleToolTip = String.Format(tooltip, name)
            End If
        End If
    End Sub
    Private Sub SetActionUrl(url As String) Implements IViewPublicCallsList.SetActionUrl
        Me.HYPtoLoginPage.Visible = True
        Me.HYPtoLoginPage.NavigateUrl = BaseUrl & url
    End Sub
    Private Sub LoadCalls(items As List(Of dtoCallItemPermission)) Implements IViewPublicCallsList.LoadCalls
    ...
    End Sub
    Private Sub SendUserAction(idCommunity As Integer, idModule As Integer, idCall As Long, action As ModuleCallForPaper.ActionType) Implements IViewPublicCallsList.SendUserAction
...
#End Region
    ...
    Private Sub PublicCallsCollector_PreRender(sender As Object, e As System.EventArgs) Handles Me.PreRender
