...
Public Class TemplateMessagePreview
    Inherits PageBase
    Implements lm.Comol.Core.BaseModules.TemplateMessages.Presentation.IViewTemplatePreview

#Region "Context"
    Private _Presenter As TemplatePreviewPresenter
    Private ReadOnly Property CurrentPresenter() As TemplatePreviewPresenter
        Get
            If IsNothing(_Presenter) Then
                _Presenter = New TemplatePreviewPresenter(Me.PageUtility.CurrentContext, Me)
            End If
            Return _Presenter
        End Get
    End Property
...
#Region "Preload"
    Private ReadOnly Property PreloadModuleObject As ModuleObject Implements IViewTemplatePreview.PreloadModuleObject
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadIdOrganization As Integer Implements IViewTemplatePreview.PreloadIdOrganization
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadIdCommunity As Integer Implements IViewTemplatePreview.PreloadIdCommunity
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadModuleCode As String Implements IViewTemplatePreview.PreloadModuleCode
        Get
            Return Request.QueryString("code")
        End Get
    End Property
    Private ReadOnly Property PreloadIdModule As Integer Implements IViewTemplatePreview.PreloadIdModule
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadForPortal As Boolean Implements IViewTemplatePreview.PreloadForPortal
        Get
            Return (Request.QueryString("portal") = "true")
        End Get
    End Property
    Private ReadOnly Property PreloadIdTemplate As Long Implements IViewTemplatePreview.PreloadIdTemplate
        Get
        ...
        End Get
    End Property
    Private ReadOnly Property PreloadIdVersion As Long Implements IViewTemplatePreview.PreloadIdVersion
        Get
            If Not String.IsNullOrEmpty(Request.QueryString("idV")) AndAlso IsNumeric(Request.QueryString("idV")) Then
                Return CLng(Request.QueryString("idV"))
            Else
                Return 0
            End If
        End Get
...
#Region "Current"
    Private Property CurrentTranslations As List(Of lm.Comol.Core.TemplateMessages.Domain.dtoTemplateTranslation) Implements IViewTemplatePreview.CurrentTranslations
        Get
        ...
        End Set
    End Property
    Private Property ContainerContext As dtoSelectorContext Implements IViewTemplatePreview.ContainerContext
        Get
            Return ViewStateOrDefault("ContainerContext", New dtoSelectorContext())
        End Get
        Set(value As dtoSelectorContext)
            ViewState("ContainerContext") = value
        End Set
    End Property
    Private Property ContentModules As List(Of String) Implements IViewTemplatePreview.ContentModules
        Get
        ...
        End Set
    End Property
    Private ReadOnly Property TagTranslation As String Implements IViewTemplatePreview.TagTranslation
        Get
            Return LTplaceHolder.Text
        End Get
    End Property
    Private Property ContentIdTemplate As Long Implements IViewTemplatePreview.ContentIdTemplate
        Get
            Return ViewStateOrDefault("ContentIdTemplate", 0)
        End Get
        Set(value As Long)
            ViewState("ContentIdTemplate") = value
        End Set
    End Property
    Private Property ContentIdVersion As Long Implements IViewTemplatePreview.ContentIdVersion
        Get
            Return ViewStateOrDefault("ContentIdVersion", 0)
        End Get
        Set(value As Long)
            ViewState("ContentIdVersion") = value
        End Set
    End Property
#End Region
...
#Region "Inherits"
    ...
    Public Overrides Sub BindDati()
        ...
        Me.CurrentPresenter.InitView(PreloadIdTemplate, PreloadIdVersion)
    End Sub
...
#Region "Implements"
    Private Sub DisplaySessionTimeout() Implements IViewTemplatePreview.DisplaySessionTimeout
        Dim cContext As dtoSelectorContext = ContainerContext
        Dim idCommunity As Integer = cContext.IdCommunity
        Dim webPost As New lm.Comol.Core.DomainModel.Helpers.LogoutWebPost(PageUtility.GetDefaultLogoutPage)
        Dim dto As New lm.Comol.Core.DomainModel.Helpers.dtoExpiredAccessUrl()
        dto.Display = lm.Comol.Core.DomainModel.Helpers.dtoExpiredAccessUrl.DisplayMode.SameWindow

        dto.DestinationUrl = lm.Comol.Core.TemplateMessages.RootObject.PreviewTemplate(ContentIdTemplate, ContentIdVersion, cContext.ModuleCode, cContext.IdModule, cContext.IsForPortal, cContext.IdCommunity, cContext.IdOrganization, cContext.ObjectOwner)
    ...
    End Sub
    Private Sub DisplayNoPermission(idCommunity As Integer, idModule As Integer, moduleCode As String) Implements IViewTemplatePreview.DisplayNoPermission
    ...
    Private Function HasModulePermissions(moduleCode As String, permissions As Long, idCommunity As Integer, profileType As Integer, obj As lm.Comol.Core.DomainModel.ModuleObject) As Boolean Implements IViewTemplatePreview.HasModulePermissions
        ...
        Return result
    End Function
    Private Function GetContentPlaceHolders(modulesCodes As List(Of String)) As List(Of lm.Comol.Core.DomainModel.Helpers.TemplatePlaceHolder) Implements IViewTemplatePreview.GetContentPlaceHolders
    ...
    Private Function CreatePlaceholder(id As Integer, key As String, tag As String, prefix As String, moduleName As String, Optional isCommon As Boolean = False) As lm.Comol.Core.DomainModel.Helpers.TemplatePlaceHolder
        ...
        Return item
    End Function
    Private Sub DisplayNoTemplateFound() Implements IViewTemplatePreview.DisplayNoTemplateFound
        MLVtemplate.SetActiveView(VIWunknownTemplate)

    End Sub
    Private Sub InitializeControls(tTemplates As List(Of dtoTemplateTranslation), inUse As List(Of lm.Comol.Core.DomainModel.Languages.LanguageItem), current As lm.Comol.Core.DomainModel.Languages.LanguageItem) Implements IViewTemplatePreview.InitializeControls
    ...
    Private Sub InitializeMailSettings(settings As lm.Comol.Core.MailCommons.Domain.Messages.MessageSettings, editSender As Boolean, editSubject As Boolean, editSignature As Boolean) Implements IViewTemplatePreview.InitializeMailSettings
        DVmailSettings.Visible = True
        Me.CTRLmailSettings.InitializeControl(settings, editSender, editSubject, editSignature, False)
    End Sub

    Private Sub LoadTemplate(content As dtoTemplateTranslation) Implements IViewTemplatePreview.LoadTemplate
...
#Region "Control"
    ...
    Private Sub CTRLlanguageSelector_SelectedLanguage(l As lm.Comol.Core.DomainModel.Languages.LanguageItem) Handles CTRLlanguageSelector.SelectedLanguage
        ...
        LoadTemplate(CurrentTranslations.Where(Function(t) t.IdLanguage = l.Id AndAlso t.LanguageCode = l.Code).FirstOrDefault())
    End Sub
#End Region


    Public Sub DisplayLastVersionInfo() Implements lm.Comol.Core.BaseModules.TemplateMessages.Presentation.IViewTemplatePreview.DisplayLastVersionInfo

    End Sub

    Public Sub DisplayNoTemplateVersionFound() Implements lm.Comol.Core.BaseModules.TemplateMessages.Presentation.IViewTemplatePreview.DisplayNoTemplateVersionFound

    End Sub

    Public Sub DisplayVersionInfo(versionNumber As Integer) Implements lm.Comol.Core.BaseModules.TemplateMessages.Presentation.IViewTemplatePreview.DisplayVersionInfo
